...
import com.intellij.lombok.processors.util.LombokUtil;
...
public abstract class NArgsConstructorAnnotationProcessor extends LombokSelfClassProcessor {
  ...
  public void processElement(@NotNull PsiClass parent, @NotNull PsiClass psiClass, @NotNull List< PsiElement > result) {
    ...
    applicableFields = ContainerUtil.filter(applicableFields, new Condition<PsiField>() {
      @Override
      public boolean value(PsiField psiField) {
        return isFieldIsApplicable(psiField);
      }
    });
  ...
  private void createStaticConstructor(PsiClass parent,
                                       PsiClass psiClass,
                                       List<PsiElement> result,
                                       PsiAnnotation affectedAnnotation,
                                       List<PsiField> applicableFields) {
