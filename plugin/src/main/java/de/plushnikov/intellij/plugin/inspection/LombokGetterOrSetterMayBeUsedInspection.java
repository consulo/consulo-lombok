...
public abstract class LombokGetterOrSetterMayBeUsedInspection extends LombokJavaInspectionBase {
    ...
    private class LombokGetterOrSetterMayBeUsedVisitor extends JavaElementVisitor {
        ...
        private void warnOrFix(
            ...
            if (myHolder != null) {
                ...
                myHolder.newProblem(getClassErrorMessage(className))
                    .range(psiClass, rangeInElement)
                    .highlightType(ProblemHighlightType.GENERIC_ERROR_OR_WARNING)
                    .withFix(new LombokGetterOrSetterMayBeUsedFix(Objects.requireNonNull(className)))
                    .create();
            }
        ...
        private void warnOrFix(@Nonnull PsiField field, @Nonnull PsiMethod method) {
            if (myHolder != null) {
                myHolder.newProblem(getFieldErrorMessage(field.getName()))
                    .range(method)
                    .withFix(new LombokGetterOrSetterMayBeUsedFix(field.getName()))
                    .create();
            }
            else if (myLombokGetterOrSetterMayBeUsedFix != null) {
    ...
    private class LombokGetterOrSetterMayBeUsedFix implements LocalQuickFix {
        ...
        public void effectivelyDoFix(
            @Nonnull PsiClass aClass, @Nonnull List<Pair<PsiField, PsiMethod>> fieldsAndMethods,
            @Nonnull List<PsiField> annotatedFields
        ) {
            if (!addLombokAnnotation(aClass)) {
                return;
            }
    ...
    @Nonnull
    protected abstract LocalizeValue getFieldErrorMessage(String fieldName);

    @Nonnull
    protected abstract LocalizeValue getClassErrorMessage(String className);
    ...
    @Nonnull
    protected abstract LocalizeValue getFixName(String text);
}
